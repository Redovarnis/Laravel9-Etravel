    <!-- SIDEBAR START -->

	<div class="wrapper">

        <nav id="sidebar" class="sidebar js-sidebar">
			<div class="sidebar-content js-simplebar">
				<a class="sidebar-brand" href="http://127.0.0.1:8000/admin">
                    <span class="align-middle">E-Travel</span>
                </a>
				<ul class="sidebar-nav">
                    <li class="sidebar-header">Profile</li>
                    <li class="sidebar-item">
						<a class="sidebar-link" href="/admin">
                            <i class="align-middle" data-feather="user"></i>
                            <span class="align-middle">Dashboard</span>
                        </a>
					</li>
                    <li class="sidebar-header">
						Posts
					</li>

					<li class="sidebar-item">
						<a class="sidebar-link" href="index.html">
                            <i class="align-middle" data-feather="hash"></i>
                            <span class="align-middle">New Posts</span>
                        </a>
					</li>
                    <li class="sidebar-item">
						<a class="sidebar-link" href="index.html">
                            <i class="align-middle text-success" data-feather="check"></i>
                            <span class="align-middle">Accepted Posts</span>
                        </a>
					</li>
                    <li class="sidebar-item">
						<a class="sidebar-link" href="index.html">
                            <i class="align-middle text-warning" data-feather="activity"></i>
                            <span class="align-middle">Shipping </span>
                        </a>
					</li>
                    <li class="sidebar-item">
						<a class="sidebar-link" href="index.html">
                            <i class="align-middle text-success" data-feather="check-circle"></i>
                            <span class="align-middle">Completed Posts</span>
                        </a>
					</li>

                    <li class="sidebar-header"></li>
                    <li class="sidebar-item">
						<a class="sidebar-link" href="/admin/category">
                            <i class="align-middle text-warning" data-feather="list"></i>
                            <span class="align-middle">Categories</span>
                        </a>
					</li>
                    <li class="sidebar-item">
						<a class="sidebar-link" href="/admin/post">
                            <i class="align-middle text-info" data-feather="send"></i>
                            <span class="align-middle">Posts</span>
                        </a>
					</li>
                    <li class="sidebar-item">
						<a class="sidebar-link" href="/admin/comment">
                            <i class="align-middle text-info" data-feather="message-square"></i>
                            <span class="align-middle">Comments</span>
                        </a>
					</li>
                    <li class="sidebar-item">
						<a class="sidebar-link" href="/admin/faq">
                            <i class="align-middle text-warning" data-feather="help-circle"></i>
                            <span class="align-middle">FAQ</span>
                        </a>
					</li>
                    <li class="sidebar-item">
						<a class="sidebar-link" href="/admin/user">
                            <i class="align-middle text-success" data-feather="user"></i>
                            <span class="align-middle">Users</span>
                        </a>
					</li>
                    <li class="sidebar-item">
						<a class="sidebar-link" href="/admin/social">
                            <i class="align-middle text-info" data-feather="message-circle"></i>
                            <span class="align-middle">Social</span>
                        </a>
					</li>

                    <li class="sidebar-header">LABELS</li>
                    <li class="sidebar-item">
						<a class="sidebar-link" href="/admin/setting">
                            <i class="align-middle text-secondary" data-feather="settings"></i>
                            <span class="align-middle">Settings</span>
                        </a>
					</li>

				<div class="sidebar-cta">
					<div class="sidebar-cta-content">
						<strong class="d-inline-block mb-2">Upgrade to Pro</strong>
						<div class="mb-3 text-sm">
							Are you looking for more components? Check out our premium version.
						</div>
						<div class="d-grid">
							<a href="upgrade-to-pro.html" class="btn btn-primary">Upgrade to Pro</a>
						</div>
					</div>
				</div>
			</div>
		</nav>

        <!-- SIDEBAR END -->
